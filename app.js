  ...
  try {
    ...
    if (txRes.result && Array.isArray(txRes.result)) {
      txRes.result.forEach(t => {
        ...
        tokens[t.contractAddress.toLowerCase()] = { symbol: t.tokenSymbol, decimals: t.tokenDecimal };
      });
    }
    ...
    const assets = [{ name: 'ETH', symbol: 'ETH', amount: ethBal, price: ethPrice, total: ethTotal }];
        ...
        if ((price === null || price === undefined) && c) {
          try {
            ...
            if (pairRes && pairRes.pairs && pairRes.pairs.length) {
              ...
              if (p && p.priceUsd) price = parseFloat(p.priceUsd);
            }
          } catch (e) { /* ignore */ }
        }
        const total = price ? (val * price) : 0;
        assets.push({ name: token.symbol, symbol: token.symbol, amount: val, price: price, total: total });
      } catch (e) {
    ...
    // Render HTML con formatos:
    // price 4 decimales para BIO, USD0, USUAL, USUALX
    // ETH amount 6 decimales, resto 4 decimales
